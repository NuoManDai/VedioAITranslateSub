...
async def get_logs(
    ...
    level: Optional[str] = Query(
        None, description="按日志级别过滤: INFO, WARNING, ERROR"
    ),
    source: Optional[str] = Query(None, description="按日志来源过滤"),
    video_id: Optional[str] = Query(None, alias="videoId", description="按视频ID过滤"),
):
    """
    获取处理日志（支持增量获取）
    ...
    return log_store.get_since(last_id, limit, level, source, video_id)
