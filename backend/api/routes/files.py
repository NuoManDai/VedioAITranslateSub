...
async def get_stage_files(
    ...
    video_id: Optional[str] = Query(
        None, alias="video_id", description="Video ID for per-video output directory"
    ),
):
    """获取指定阶段的输出文件列表"""
    ...
    for file_def in STAGE_OUTPUT_FILES[stage_name]:
        # Resolve path: replace "output/" prefix with "output/{video_id}/" when video_id is provided
        raw_path = file_def["path"]
        if video_id and raw_path.startswith("output/"):
            resolved_path = f"output/{video_id}/{raw_path[len('output/') :]}"
        else:
            resolved_path = raw_path

        file_path = PROJECT_ROOT / resolved_path
    ...
    try:
        for item in sorted(folder_path.iterdir()):
            file_type = (
                item.suffix.lower()[1:]
                if item.suffix
                else ("folder" if item.is_dir() else "unknown")
            )
            files.append(
                {
                    "name": item.name,
