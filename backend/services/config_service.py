...
class ConfigService:
        ...
        for key, value in update_dict.items():
            ...
            if value is not None:
                if (
                    ...
                    and yaml_key in raw_yaml
                    and isinstance(raw_yaml.get(yaml_key), dict)
                ):
    ...
    async def validate_api_key(
        self, key: str, base_url: str, model: str
    ) -> ApiValidateResponse:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return ApiValidateResponse(
                        valid=False, message=f"Model {model} not found"
                    )
                else:
                    return ApiValidateResponse(
                        valid=False,
    ...
    def _yaml_to_config(self, data: dict) -> Configuration:
        ...
        if style_data:
            ...
            translation_data = style_data.get("translation", {})
            original_data = style_data.get("original", {})
            layout_data = style_data.get("layout", {})

            style_config = SubtitleDisplayStyle(
                translation=SubtitleStyleConfig(
                    font_size=translation_data.get("font_size", 18),
                    font_color=translation_data.get("font_color", "#FFFFFF"),
                    bg_color=translation_data.get("bg_color", "rgba(0,0,0,0.75)"),
                    outline_color=translation_data.get("outline_color", "#000000"),
                    outline_width=translation_data.get("outline_width", 1),
                )
                if translation_data
                else None,
                original=SubtitleStyleConfig(
                    font_size=original_data.get("font_size", 14),
                    font_color=original_data.get("font_color", "#CCCCCC"),
                    bg_color=original_data.get("bg_color", "rgba(0,0,0,0.6)"),
                    outline_color=original_data.get("outline_color", "#000000"),
                    outline_width=original_data.get("outline_width", 1),
                )
                if original_data
                else None,
                layout=SubtitleLayoutConfig(
                    margin_bottom=layout_data.get("margin_bottom", 40),
                    line_spacing=layout_data.get("line_spacing", 8),
                )
                if layout_data
                else None,
            )
        ...
        return Configuration(
            ...
            max_split_length=data.get(
                "max_split_length", 12 if source_language == "ja" else 20
            ),
    ...
    def _config_to_yaml(self, config: dict) -> dict:
        ...
        if "whisper" in config and config["whisper"]:
            ...
            if config["whisper"].get("whisperX_302_api_key"):
                yaml_data["whisper"]["whisperX_302_api_key"] = config["whisper"][
                    "whisperX_302_api_key"
                ]
            if config["whisper"].get("elevenlabs_api_key"):
                yaml_data["whisper"]["elevenlabs_api_key"] = config["whisper"][
                    "elevenlabs_api_key"
                ]
        else:
