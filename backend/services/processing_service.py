...
class TqdmCapture(io.StringIO):
    ...
    def __init__(self, log_store, source: str, job_id: str, original_stream, stage_name: str = ''):
        super().__init__()
    ...
    def write(self, text: str):
        ...
        with self._lock:
            ...
            if tqdm_match:
                ...
                progress_match = re.search(r'Progress:\s*(\d+(?:\.\d+)?)%', self._buffer)
                if progress_match:
            ...
            if '\n' in self._buffer or '\r' in self._buffer:
                ...
                for line in lines[:-1]:
                    ...
                    stripped_of_borders = re.sub(r'[─│╭╮╰╯┌┐└┘├┤┬┴┼═║╔╗╚╝╠╣╦╩╬┏┓┗┛┃┡┩━╇╈╋\s]+', '', clean_line)
                    ...
                    if self._important_regex.search(clean_line):
                        ...
                        display_line = re.sub(r'^[│┃]\s*', '', display_line)  # Leading border
                        display_line = re.sub(r'\s*[│┃]$', '', display_line)  # Trailing border
                        display_line = re.sub(r'\s*[│┃]\s*', ' | ', display_line)  # Middle borders to readable separator
                        display_line = display_line.strip()
                        
                        # Remove timestamp prefix if present
                        timestamp_match = re.match(r'^\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}[,\d]*\s*-?\s*', display_line)
                        if timestamp_match:
...
class ProcessingService:
    ...
    async def run_subtitle_processing(self, job: ProcessingJob, video: Video):
        ...
        try:
            ...
            if state.is_cancel_requested():
                log_store.warning("字幕处理被用户取消", source='subtitle', job_id=job.id)
                return
    ...
    async def _run_stage(self, job: ProcessingJob, stage_name: str, stage_func):
        ...
        def run_with_capture():
            ...
            capture_stderr = TqdmCapture(log_store, job.job_type, job.id, original_stderr, stage_name)
            capture_stdout = TqdmCapture(log_store, job.job_type, job.id, original_stdout, stage_name)
            try:
    ...
    def detect_completed_stages(self, job_type: str = 'subtitle') -> dict:
        ...
        if job_type == 'subtitle':
            stage_order = ['asr', 'split_nlp', 'split_meaning', 'summarize', 'translate', 'split_sub', 'gen_sub', 'merge_sub']
        else:
            stage_order = ['audio_task', 'dub_chunks', 'refer_audio', 'gen_audio', 'merge_audio', 'dub_to_vid']
    ...
    def restore_job_state(self, job_type: str = 'subtitle') -> Optional['ProcessingJob']:
        """
    ...
    def cleanup_subtitle_files(self) -> dict:
        ...
        logger.info(f"Subtitle cleanup: cleaned {len(cleaned_paths)} items, preserved {len(preserved_paths)} items")
    ...
    def cleanup_dubbing_files(self) -> dict:
        ...
        logger.info(f"Dubbing cleanup: cleaned {len(cleaned_paths)} items, preserved {len(preserved_paths)} items")
    ...
    def cleanup_all_files(self) -> dict:
        ...
        logger.info(f"Full cleanup: cleaned {len(cleaned_paths)} items, preserved {len(preserved_paths)} original videos")
