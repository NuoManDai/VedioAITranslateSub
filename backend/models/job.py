...
class ProcessingJob(BaseModel):
    ...
    stages: list[ProcessingStage] = Field(default_factory=list, description="各阶段状态列表")
    ...
    def update_stage(self, stage_name: str, status: str, progress: Optional[float] = None, error: Optional[str] = None, message: Optional[str] = None):
        """更新阶段状态"""
    ...
    def _calculate_progress(self):
        ...
        if running > 0:
            running_stage = next((s for s in self.stages if s.status == 'running'), None)
